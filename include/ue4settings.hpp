...
std::array UE4settingsArray = {
	...
	*/
	...
	UESetting{"r.DynamicRes.OutlierThreshold",						"Ignore frame timing that have Game thread or render thread X time more than frame budget.", 1}, //Default 0

	UESetting{"r.DynamicRes.IncreaseAmortizationBlendFactor",		"Amortization blend factor when scale resolution back up to reduce resolution fraction oscillations.", 2}, //Default 0.9

	UESetting{"r.DynamicRes.HistorySize",							"Number of frames keept in the history.", 1}, //Default 16

	UESetting{"r.DynamicRes.GPUTimingMeasureMethod",				"Selects the method to use to measure GPU timings.\n"
																	" 0: Same as stat unit (default);\n"
																	" 1: Timestamp queries.", 1},

	UESetting{"r.DynamicRes.MinResolutionChangePeriod",				"Minimal number of frames between resolution changes, important to avoid input sample position interferences in TAA upsample.", 1}, //Default 8

	UESetting{"r.DynamicRes.ChangePercentageThreshold",				"Minimal increase percentage threshold to alow when changing resolution.", 2}, //Default 2.0

	UESetting{"r.DynamicRes.FrameWeightExponent",					"Recursive weight of frame N-1 against frame N.", 2}, //Default 0.9

	UESetting{"r.DynamicRes.MaxConsecutiveOverbudgetGPUFrameCount",	"Maximum number of consecutive frame tolerated over GPU budget.", 1}, //Default 2

	UESetting{"r.DynamicRes.CPUTimeHeadRoom",						"Head room for the threads compared GPU time to avoid keep getting resolution fraction shrinking down when CPU bound (in milliseconds).", 2}, //Default 1.0

	UESetting{"r.DynamicRes.TargetedGPUHeadRoomPercentage",			"Targeted GPU headroom (in percent from r.DynamicRes.FrameTimeBudget).", 2}, //Default 10.0
